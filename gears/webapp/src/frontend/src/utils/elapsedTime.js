...
function dateDiff(toStringDateOrNumber, fromStringDateOrNumber = new Date()) {
	...
	if (!(to instanceof Date) || isNaN(to)) {
		throw new RangeError(
			`To date is required to be a valid ISO 8601 string, milliseconds since epoch or Date object, received ${toStringDateOrNumber}`
		);
	}

	if (!(from instanceof Date) || isNaN(from)) {
		throw new RangeError(
			`From date is required to be a valid ISO 8601 string, milliseconds since epoch or Date object, received ${fromStringDateOrNumber}`
		);
	}
