...
function looksLikeUserObject(obj) {
	return (
		obj &&
		typeof obj === "object" &&
		(isNumber(obj.id) || isStringWithLength(obj.hashid) || isStringWithLength(obj.id /* could be a hash */))
	);
}
...
class UpdatesCollection {
	...
	pushDifferences(
		oldObject,
		newObject,
		sensitiveKeys = ["pass", "password", "token", "secret", "credentials", "credentialSecret", "cookieSecret"]
	) {
...
function generateUpdates(o1, o2, sensitiveKeys) {
	...
	const diffToArray = (diff) => {
		return Object.entries(diff || {}).map(([key, val]) => {
			return updatesObject(key, val.old, val.new, val.dif);
		});
	};
