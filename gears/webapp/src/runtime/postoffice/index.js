...
module.exports = fp(
	async function (app, _opts) {
		...
		const mailDefaults = {
			from: app.config.email.from ? app.config.email.from : '"Transcendence Platform" <no-reply@transcendence.com>',
		};
		...
		if (isConfigured) {
			poStartupCheck = setInterval(
				...
				1000 * 60 * 5
			); // check every 5 minutes until successful
		}
		...
		async function send(user, templateName, context) {
			...
			templateContext.support = /^https:\/\/(app.flowfuse.com|forge.flowfuse.dev)/g.test(forgeURL)
				? "https://flowfuse.com/support/"
				: null;
