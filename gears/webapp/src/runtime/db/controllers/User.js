...
module.exports = {
	...
	authenticateCredentials: async function (app, username, password) {
		...
		const user = await app.db.models.User.findOne({
			where: where(fn("lower", col(column)), username.toLowerCase()),
			attributes: ["password"],
		});
	...
	generatePendingEmailChangeToken: async function (app, user, newEmailAddress) {
		...
		return jwt.sign(
			{ sub: user.email, id: user.hashid, change: newEmailAddress, aud: "update-user-email", exp: expiresAt },
			signingHash
		);
	},

	sendPendingEmailChangeEmail: async function (app, user, newEmailAddress) {
		const pendingEmailChangeToken = await app.db.controllers.User.generatePendingEmailChangeToken(
			user,
			newEmailAddress
		);
		const recipient = {
		...
		await app.postoffice.send(recipient, "PendingEmailChange", {
			oldEmail: user.email,
	...
	sendEmailChangedEmail: async function (app, recipient, oldEmailAddress, newEmailAddress) {
		await app.postoffice.send(recipient, "EmailChanged", {
