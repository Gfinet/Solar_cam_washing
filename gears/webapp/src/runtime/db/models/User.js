...
module.exports = {
	...
	associations: function (M) {
		...
		this.belongsToMany(M.ChatChannel, {
			through: "ChatMember",
			as: "channels",
			foreignKey: "userId",
		});
		this.hasMany(M.ChatMessage, {
			foreignKey: "userId",
			as: "messages",
		});
		this.hasMany(M.ChatChannel, { foreignKey: "ownerId" /*, as: 'ownedChannels'*/ });
	},
		...
		return {
			static: {
				...
				byUsernameOrEmail: async (name) => {
					return this.findOne({
						where: where(fn("lower", col(/.+@.+/.test(name) ? "email" : "username")), name.toLowerCase()),
					});
				},
