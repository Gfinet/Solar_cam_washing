			...
			static: {
				...
				getFilterAndAssociations: async (entityType, entityId, pagination) => {
                    ...
                    */
					...
					const addApplicationScope = async (
						teamId,
						applicationId = null,
						includeInstances = false,
						includeApplicationDevices = false,
						includeInstanceDevices = false
					) => {
						let applicationIds = [];
						if (applicationId) {
							const _application = await M.Application.findOne({
								where: { id: applicationId },
								attributes: ["id", "hashid", "name", "TeamId"],
							});
							...
							const _applications = await M.Application.findAll({
								where: clause,
								attributes: ["id", "hashid", "name", "TeamId"],
							});
					...
					const addInstanceScope = async (
						...
						if (instanceId) {
							if (Array.isArray(instanceId)) {
								const _instances = await M.Project.findAll({
									where: { id: { [Op.in]: instanceId } },
									attributes: ["id", "name", "ApplicationId", "TeamId", "state"],
								});
								...
								const _instance = await M.Project.findOne({
									where: { id: instanceId },
									attributes: ["id", "hashid", "name", "ApplicationId", "TeamId", "state"],
								});
					...
					const addDeviceScope = async (teamId, applicationId = null, instanceId = null) => {
						...
						const _devices = await M.Device.findAll({
							where: clause,
							attributes: [
								"id",
								"hashid",
