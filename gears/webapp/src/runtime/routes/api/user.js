...
module.exports = async function (app) {
	...
	app.put(
		...
		async (request, reply) => {
			try {
				await app.db.controllers.User.changePassword(
	...
	app.get(
			...
			schema: {
				...
				response: {
					200: {
						...
						properties: {
							count: { type: "number" },
							sessions: { $ref: "PersonalAccessTokenSummaryList" },
						},
					},
					"4xx": {
						$ref: "APIError",
		...
		async (request, reply) => {
			try {
				const tokens = await app.db.models.AccessToken.getPersonalAccessTokens(request.session.User);
				reply.send({
					sessions: app.db.views.AccessToken.personalAccessTokenSummaryList(tokens),
					count: tokens.length,
				});
			} catch (err) {
				const resp = { code: "unexpected_error", error: err.toString() };
