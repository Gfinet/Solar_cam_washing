 ...
 * @param {import("fastify").FastifyInstance & {config: { port: number; host: string; }}} app
 * @param {Object} opts
 * @param {Function} done
 */
	...
	async function verifySession(request, reply) {
		if (request.sid) {
			...
			if (request.session && request.session.User) {
				const emailVerified =
					!app.postoffice.enabled() ||
					request.session.User.email_verified ||
					request.routeOptions.config.allowUnverifiedEmail;
				const passwordNotExpired =
					!request.session.User.password_expired || request.routeOptions.config.allowExpiredPassword;
	...
	app.post(
		"/account/login",
		{
			...
			schema: {
				...
				description:
					"Log in to the platform. If SSO is enabled for this user, the response will prompt the user to retry via the SSO login mechanism.",
		...
		async (request, reply) => {
			...
			if (!request.body.password) {
				...
				const result = await app.db.controllers.User.authenticateCredentials(
		...
		async (request, reply) => {
			...
			if (!app.postoffice.enabled()) {
				const resp = {
					...
					error: "user registration not enabled - email not configured",
				};
			...
			try {
				...
				await app.postoffice.send(newUser, "VerifyEmail", { token: verificationToken });
	...
	app.post(
		...
		async (request, reply) => {
			...
			if (!request.session.User.email_verified) {
				...
				await app.postoffice.send(request.session.User, "VerifyEmail", {
		...
		async (request, reply) => {
			...
			if (user) {
				if (app.postoffice.enabled()) {
					...
					app.postoffice.send(user, "PasswordReset", {
